...
vec3 get_parameter_for_color_stage(int idx, vec4 konst) {
	switch (idx) {
		...
		case 14: // return konst.xyz; 
		return vec3(0.0);
		case 15: return vec3(0.0, 0.0, 0.0);
	}
}

vec3 get_parameter_for_alfa_stage(int idx, vec4 konst) {
	switch (idx) {
		...
		case 4: return vec3(1.0);
		case 5: return ras.aaa;
		case 6: // return konst.xyz;
			return vec3(1.0);
...
void main(void) {
	...
	for (int i = 0; i < num_tev_stages; i++) {
		vec3 ca = get_parameter_for_color_stage(in_color_a[i], konst_a);
		vec3 cb = get_parameter_for_color_stage(in_color_b[i], konst_b);
		vec3 cc = get_parameter_for_color_stage(in_color_c[i], konst_c);
		vec3 cd = get_parameter_for_color_stage(in_color_d[i], konst_d);
		vec3 aa = get_parameter_for_alfa_stage(in_alfa_a[i],  konst_a);
		vec3 ab = get_parameter_for_alfa_stage(in_alfa_b[i],  konst_b);
		vec3 ac = get_parameter_for_alfa_stage(in_alfa_c[i],  konst_c);
		vec3 ad = get_parameter_for_alfa_stage(in_alfa_d[i],  konst_d);
		...
		color_regs[color_dest[i]].xyz = last_color_dest;
