...
final class DspJit {
    ...
    JitExitReason run_cycles(DspState* state, u32 max_cycles) {
        ...
        while (cycles_executed < max_cycles) {
            ...
            if (state.pc == old_pc && old_pc == 0x00e8) {
                dipshit = true;
            }
    ...
    JitExitReason execute_compiled_block(DspJitFunction func, DspState* state) {
        ...
        // import std.stdio;
        // writefln("DSP executing PC=0x%04x", state.pc);

        // if (dipshit) {
            // writefln("DSP has entered dipshit mode at PC=0x%04x", state.pc);
        // }
        ...
        //     import std.stdio;
        //     writef("DSP call stack changed (sp: %d -> %d) at PC=0x%04x: [", call_stack_sp_before, call_stack_sp_after, state.pc);
        //     for (int i = 0; i < call_stack_sp_after; i++) {
        //         if (i > 0) writef(", ");
        //         writef("0x%04x", state.call_stack.data[i]);
        //     }
        //     writefln("]");
        // }

        // if ((state.pc >= 0xdb6 && state.pc <= 0xdec)) {
